...
# ==============================
# ÉQUIVALENCES CO2 – Voiture / Avion
# ==============================
EF_CAR_KG_PER_KM = 0.15        # 150 gCO2/km (hypothèse)
PNY_AR_TONNES    = 1.65        # 1 A/R Paris–New York (classe éco) ~1.65 tCO2 (ajustable)
PARIS_LYON_AR_KM = 950         # A/R Paris–Lyon ~950 km (repère voiture)

def car_km_from_tonnes(t):
    """Convertit des tonnes CO₂ en kilomètres voiture équivalents."""
    return (t * 1000.0) / EF_CAR_KG_PER_KM  # t -> kg ; / kg/km -> km

def flights_from_tonnes(t, flight_t=PNY_AR_TONNES):
    """Convertit des tonnes CO₂ en nombre d'A/R Paris–New York équivalents."""
    return t / flight_t

# Table d’équivalences (≈30 paliers)
PALIER_T = [0.5] + list(range(1, 31))  # 0.5, 1, 2, ..., 30
# on s’assure que 15 et 25 sont bien dedans (ils le sont déjà)
if 15 not in PALIER_T: PALIER_T.append(15)
if 25 not in PALIER_T: PALIER_T.append(25)
PALIER_T = sorted(PALIER_T)

equivalences = []
for t in PALIER_T:
    km = car_km_from_tonnes(t)
    vols = flights_from_tonnes(t)
...
def nearest_equivalence(t):
    """Renvoie l’entrée d’équivalence au palier le plus proche de t (en tCO2)."""
    return min(equivalences, key=lambda e: abs(e["gain_t"] - t))

# --- Affichage pour l’utilisateur, selon le cas ---
eq = nearest_equivalence(gain_co2)

if gain_co2 < PNY_AR_TONNES:
    # Cas < 1 A/R Paris–NY -> on parle en voiture
    km_eq = car_km_from_tonnes(gain_co2)
    paris_lyon_eq = km_eq / PARIS_LYON_AR_KM
    st.subheader("🚗 Équivalences (repères concrets)")
    st.write(
    ...
    # Cas ≥ 1 A/R Paris–NY -> on parle en vols
    n_vols = flights_from_tonnes(gain_co2)
    st.subheader("✈️ Équivalences (repères concrets)")
    st.write(
        f"• Votre économie de **{gain_co2:.2f} tCO₂** "
        f"équivaut à **~{n_vols:.2f} A/R Paris–New York (classe éco)**."
    )

# Palier le plus proche (à afficher en plus)
st.caption(
    f"Équivalence la plus proche (palier **{eq['gain_t']} t**): "
    f"~{eq['car_km']:,} km en voiture "
